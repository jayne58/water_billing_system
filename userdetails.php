...
$sql = "SELECT fname, lname, phone_number, meter_number, residency, usertype  FROM userdetails";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
  echo "<table class='table-bordered'><tr><th>fname</th><th>lname</th><th>phone_number</th><th>meter_number</th><th>residency</th><th>usertype</th></tr>";
  // output data of each row
  while($row = $result->fetch_assoc()) {
    echo "<tr><td>".$row["fname"]."</td><td>".$row["lname"]."</td><td>".$row["phone_number"]."</td><td>".$row["meter_number"]."</td><td>".$row["residency"]."</td><td>".$row["usertype"]."</td></tr>";
  }
  echo "</table>";
} else {
  echo "0 results";
